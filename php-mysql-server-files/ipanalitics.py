...
def connect_db(sql):
    ...
    cnx = pymysql.connect(user=db_user, password=db_pass, host='127.0.0.1', database='minikron')
    cursor = cnx.cursor()
    cursor.execute(sql)
    ret = cursor
...
def login_form(user, password):
    log_report("    <SQL> - SELECT id_h FROM hosty WHERE host_nazw='" + user + "' AND haslo='" + password + "';",
               "green")
...
class ip_Analitics(BaseHTTPRequestHandler):
    ...
    def do_HEAD(self):
        ...
        connect_db(error_raport("PROTOCOL", self.address_string(), "", "", "", self.command + ":" + self.path,
...
def run_ia():
    ...
    httpd = HTTPServer(('', port_num), handler)
    try:
